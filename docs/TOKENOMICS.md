...
The current QraftCoin source implements Qraft Coin (QFC). Deployment is pending.
...
| Current deployment | Pending |
| Admin | None |
| Upgradeable | No |
| Current on-chain supply | Pending deployment; burns can reduce initial supply |
...
There is no asserted price, market capitalization, liquidity, exchange listing, investor allocation, yield or investment return. Experimental QFC tokens have no guaranteed market value.
