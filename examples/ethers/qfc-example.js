// Read-only example for the future QFC Sepolia deployment.
...
async function getQFCBalance(walletAddress) {
  ...
  try {
    ...
    const [balance, decimals, symbol, name] = await Promise.all([
      token.balanceOf(walletAddress), token.decimals(), token.symbol(), token.name()
    ]);
    if (symbol !== "QFC") throw new Error("Expected QFC token symbol; check QFC_CONTRACT_ADDRESS.");
    return { balance: ethers.formatUnits(balance, decimals), symbol, name };
  } finally {
    provider.destroy();
  }
}
...
if (require.main === module) {
  getQFCBalance(process.argv[2]).then(console.log).catch(() => {
    console.error("Balance lookup failed. Expected QFC token symbol and contract bytecode on Sepolia. Check wallet, contract address, Sepolia RPC and connectivity. No signing key is needed.");
    process.exitCode = 1;
  });
}
