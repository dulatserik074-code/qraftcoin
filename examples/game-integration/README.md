...
1. Wait for the separately authorized QFC Sepolia deployment and obtain its verified address.
...
Expected output is an object containing a formatted balance and the actual on-chain symbol. The example rejects any token whose symbol is not QFC. Never convert the balance through JavaScript Number for transaction arithmetic.
