...
import { explorerUrl, NETWORK_NAME, PAYMENT_ABI, QFT_ABI, QFT_TOKEN_ADDRESS, QRAFT_PAYMENT_ADDRESS, SUPPORTED_CHAIN_ID } from "@/lib/contract";
...
function friendlyError(error: unknown, fallback: string) {
  ...
  if (message.includes("ERC20InsufficientBalance")) return "Your QFT balance is too low.";
  if (message.includes("ERC20InsufficientAllowance")) return "The QFT allowance is too low.";
...
export default function Home() {
  ...
  async function submit() {
    try {
      ...
      if (value > balance) throw new Error("Your QFT balance is too low.");
      ...
      setStatus({ type: "success", text: mode === "send" ? "QFT sent successfully." : "QFT burned successfully.", hash: tx.hash });
  ...
  async function approvePayment() {
    try {
      ...
      if (preview.payment > balance) throw new Error("Your QFT balance is too low.");
  ...
  async function pay() {
    try {
      if (!preview) throw new Error("Enter a valid payment amount.");
      if (preview.payment > balance) throw new Error("Your QFT balance is too low.");
      if (allowance < preview.payment) throw new Error("Approve this QFT amount before paying.");
  ...
  return <><Header/><main className="shell">
    ...
    <section className="dashboard">
      <article className="wallet-card"><div className="wallet-top"><span>QRAFT COIN</span><span className="symbol">QFT</span></div><div className="balance-label">YOUR BALANCE</div><div className="balance">{displayUnits(balance)}<small>QFT</small></div><div className="no-price">Estimated value <strong>Not available</strong></div><div className="wallet-bottom"><div><span>NETWORK</span><b><i className="green"/>{network}</b></div><div><span>ADDRESS</span>{walletLink ? <a href={walletLink} target="_blank" rel="noreferrer">{short} ↗</a> : <b>{short}</b>}</div></div></article>
      <article className="action-card"><div className="tabs"><button disabled={pending} className={mode === "send" ? "active" : ""} onClick={() => setMode("send")}><ArrowUpRight size={18}/> Send QFT</button><button disabled={pending} className={mode === "burn" ? "active burn" : ""} onClick={() => setMode("burn")}><Flame size={18}/> Burn QFT</button></div>
        {!account ? <div className="connect-state"><Wallet size={36}/><h2>Your wallet is the key</h2><p>Connect MetaMask to view your balance and manage QFT.</p><button className="primary" onClick={connect}>Connect Wallet</button></div> : <div className="form">
          ...
          <label>Amount<div className="amount-input"><input value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00" inputMode="decimal" disabled={pending}/><span>QFT</span></div></label>
          <button className={`primary ${mode === "burn" ? "danger" : ""}`} onClick={submit} disabled={pending || !onSupportedChain}>{status.type === "loading" ? <RefreshCw className="spin" size={18}/> : mode === "send" ? <ArrowUpRight size={18}/> : <Flame size={18}/>} {status.type === "loading" ? "Processing…" : mode === "send" ? "Send QFT" : "Burn QFT"}</button>
        </div>}{renderStatus(status)}</article>
    </section>
    <section className="payment-card"><div className="payment-heading"><div><span className="eyebrow">SERVICE PAYMENT</span><h2>Pay with QFT</h2><p>One approval, then a transparent on-chain payment.</p></div><Zap/></div>
      {!account ? <button className="primary" onClick={connect}>Connect Wallet</button> : <div className="payment-layout"><div className="form payment-form"><label>Payment amount<div className="amount-input"><input value={payAmount} onChange={e => { setPayAmount(e.target.value); setPayStatus(IDLE); }} placeholder="100.00" inputMode="decimal" disabled={pending}/><span>QFT</span></div></label><div className="allowance"><span>Current allowance</span><strong>{displayUnits(allowance)} QFT</strong></div></div>
        <div className="payment-preview"><div><span>Payment</span><strong>{preview ? displayUnits(preview.payment, 18) : "—"} QFT</strong></div><div><span>Treasury receives</span><strong>{preview ? displayUnits(preview.treasury, 18) : "—"} QFT</strong></div><div className="burn-row"><span>Burned</span><strong>{preview ? displayUnits(preview.burned, 18) : "—"} QFT</strong></div></div>
        <div className="payment-actions">{preview && allowance < preview.payment ? <button className="primary" onClick={approvePayment} disabled={pending || !onSupportedChain}>Approve {displayUnits(preview.payment, 18)} QFT</button> : <button className="primary" onClick={pay} disabled={pending || !preview || !onSupportedChain}><Zap size={18}/>{payStatus.type === "loading" ? "Processing…" : "Pay"}</button>}</div></div>}{renderStatus(payStatus)}
    </section>
    <section className="metrics four"><div><span>TOTAL SUPPLY</span><strong>{supply === null ? "—" : displayUnits(supply, 2)} QFT</strong></div><div><span>QFT CONTRACT</span>{tokenLink ? <a href={tokenLink} target="_blank" rel="noreferrer">{QFT_TOKEN_ADDRESS.slice(0, 8)}…{QFT_TOKEN_ADDRESS.slice(-6)} ↗</a> : <strong>Not configured</strong>}</div><div><span>PAYMENT CONTRACT</span>{paymentLink ? <a href={paymentLink} target="_blank" rel="noreferrer">{QRAFT_PAYMENT_ADDRESS.slice(0, 8)}…{QRAFT_PAYMENT_ADDRESS.slice(-6)} ↗</a> : <strong>Not configured</strong>}</div><div><ShieldCheck/><p><b>Fixed by code</b><br/>No additional minting</p></div></section>
    <p className="disclaimer">QFT is currently a test token. Market price is not guaranteed.</p>
  </main></>;
}
